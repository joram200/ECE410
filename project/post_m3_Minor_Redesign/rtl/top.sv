`timescale 1ns/1ps
// =============================================================================
// top.sv — post_m3_Minor_Redesign: Option B Kalman Update Accelerator
// Changes from m3:
//   1. Register map redesign:
//        A_REG[0]   = z        (0x10)  WO
//        A_REG[1:3] = x_in     (0x18–0x28) WO
//        A_REG[4:6] = x_out    (0x30–0x40) RO  ← new: was C_REG[0:2]
//        B_REG[0:8] = P_in     (0x58–0x98) WO  (unchanged)
//        C_REG[0:8] = P_out    (0xA0–0xE0) RO  ← all 9 elements (was only 6)
//        R_REG      = R        (0xE8)      R/W ← new: replaces hardcoded 5.0
//   2. kalman_update: r_val input port replaces R_CONST localparam
//   3. NR pipeline: FSM 11→14 states (NR0_A/B, NR1_A/B, NR2_A/B);
//      sx_pipe and nr_x_pipe break the 92-level critical path into ~30-level stages
// =============================================================================
...
//
// Register map (8-byte stride, addr[7:3] = index):
//   0x00  idx 0   CTRL    R/W  [0]=start, [1]=soft_rst
//   0x08  idx 1   STAT    RO   [0]=done,  [1]=busy
//   0x10  idx 2   A_REG[0] WO  z (measurement scalar)
//   0x18  idx 3   A_REG[1] WO  x_in[0]
//   0x20  idx 4   A_REG[2] WO  x_in[1]
//   0x28  idx 5   A_REG[3] WO  x_in[2]
//   0x30  idx 6   A_REG[4] RO  x_out[0]  ← via core_x_out[0]
//   0x38  idx 7   A_REG[5] RO  x_out[1]  ← via core_x_out[1]
//   0x40  idx 8   A_REG[6] RO  x_out[2]  ← via core_x_out[2]
//   0x48  idx 9   A_REG[7] —   unused
//   0x50  idx 10  A_REG[8] —   unused
//   0x58  idx 11  B_REG[0] WO  P_in[0,0]
//   ...
//   0x98  idx 19  B_REG[8] WO  P_in[2,2]
//   0xA0  idx 20  C_REG[0] RO  P_out[0,0]
//   ...
//   0xE0  idx 28  C_REG[8] RO  P_out[2,2]
//   0xE8  idx 29  R_REG   R/W  measurement noise covariance R (default 5.0)
// -----------------------------------------------------------------------------
...
// =============================================================================
// top — post_m3_Minor_Redesign integration top
//
