...
// =============================================================================
// Module:      compute_core
// Description: Kalman Filter Hardware Compute Core.
//              Implements kf.update() for the 1-D projectile motion filter
//              (n=3 state, m=1 measurement).  Reference model:
...
// Protocol:    Pulse valid_in with a new measurement on y.  valid_out pulses
//              for exactly one cycle when x_hat_0/1/2 hold the corrected state.
//
// -----------------------------------------------------------------------------
// Clock Domain
// -----------------------------------------------------------------------------
...
//   Behavior:     On the rising edge of clk while rst=1, all registers return
//                 to their initial values (state machine to IDLE, covariance
//                 matrix to the P-init constants, all others to 0 / FP_ONE).
//                 rst must be de-asserted at least one full cycle before the
//                 first valid_in pulse.
//
// -----------------------------------------------------------------------------
// Port List
// -----------------------------------------------------------------------------
...
//   -----------  -----  -------------  ----------------------------------------
//   clk          in     1              System clock.  All registers sample on
//                                      the rising edge.
//   rst          in     1              Synchronous active-HIGH reset.
//   valid_in     in     1              Pulse HIGH for one cycle to submit a new
//                                      scalar measurement on y.  Ignored while
//                                      the FSM is not in IDLE.
//   y            in     DATA_WIDTH     Scalar measurement in Q16.16 fixed-point.
//                                      Sampled only when valid_in is asserted.
//   x_hat_0      out    DATA_WIDTH     Corrected position estimate (Q16.16).
//                                      Valid while valid_out is HIGH.
//   x_hat_1      out    DATA_WIDTH     Corrected velocity estimate (Q16.16).
//                                      Valid while valid_out is HIGH.
//   x_hat_2      out    DATA_WIDTH     Corrected acceleration estimate (Q16.16).
//                                      Valid while valid_out is HIGH.
//   valid_out    out    1              Pulses HIGH for one cycle when
//                                      x_hat_0/1/2 hold the corrected state.
// =============================================================================
...
module compute_core #(
    ...
    // -----------------------------------------------------------------------
    // Main FSM
    // Synchronous reset, active-HIGH (rst).  All registers clear on rst=1.
    // -----------------------------------------------------------------------
